...
export const openApiDocument = {
  ...
  security: [],
  tags: ["System", "Sources", "Videos", "Downloads", "Jobs", "Playback", "Cache", "Tunarr", "Settings", "Logs", "Thumbnails", "Channels"].map((name) => ({ name })),
  paths,
  components: {
    ...
    schemas: {
      ...
      Job: { type: "object", required: ["id", "type", "status", "attempts", "maxAttempts", "runAfter"], additionalProperties: true, properties: { id: { type: "string" }, type: { type: "string", enum: ["metadata", "thumbnail", "sync", "download", "cache", "retag", "tunarr_publish", "tunarr_refresh", "render", "ingest_local_scan", "channel_publish"] }, status: { type: "string", enum: ["queued", "running", "complete", "failed", "cancelled"] }, sourceId: nullableString, videoId: nullableString, channelId: nullableString, mediaItemId: nullableString, attempts: { type: "integer" }, maxAttempts: { type: "integer" }, error: nullableString, runAfter: dateTime, stoppable: { type: "boolean" } } },
      ...
      Settings: { type: "object", required: ["id", "mediaBaseDirectory", "tunarrUrl", "cacheMaxMegabytes", "cacheMaxAgeDays", "logRetentionDays", "defaultVideoQuality", "jobsPaused", "aiProvider"], additionalProperties: true, properties: { id: { type: "integer", const: 1 }, mediaBaseDirectory: { type: "string" }, tunarrUrl: { type: "string", format: "uri" }, cacheMaxMegabytes: { type: "integer" }, cacheMaxAgeDays: { type: "integer" }, logRetentionDays: { type: "integer" }, defaultVideoQuality: videoQuality, jobsPaused: { type: "boolean" }, aiProvider: { type: "string", enum: ["auto", "anthropic", "openai"] }, pathMappings: { type: "array", items: pathMapping } } },
      ...
      UpdateSettingsRequest: { type: "object", minProperties: 1, additionalProperties: false, properties: { mediaBaseDirectory: { type: "string", minLength: 1 }, tunarrUrl: { type: "string", format: "uri" }, cacheMaxMegabytes: { type: "integer", minimum: 128, maximum: 10000000 }, cacheMaxAgeDays: { type: "integer", minimum: 1, maximum: 3650 }, logRetentionDays: { type: "integer", minimum: 1, maximum: 3650 }, defaultVideoQuality: videoQuality, musicbrainzContactEmail: { anyOf: [{ type: "string", format: "email" }, { type: "null" }] }, aiProvider: { type: "string", enum: ["auto", "anthropic", "openai"] }, pathMappings: { type: "array", maxItems: 50, items: pathMapping } } },
      ...
      UpdateChannelRequest: { type: "object", minProperties: 1, additionalProperties: false, properties: { name: { type: "string", minLength: 1, maxLength: 160 }, templateId: { type: "string", minLength: 1 }, programmingOrder: { type: "string", enum: ["manual", "oldest", "newest", "random", "ai"] }, logoAssetPath: nullableString, tunarrRequestedChannelNumber: { type: ["integer", "null"], minimum: 1 }, aiProgrammingInstructions: { anyOf: [{ type: "string", maxLength: 4000 }, { type: "null" }] }, aiProvider: { anyOf: [{ type: "string", enum: ["anthropic", "openai"] }, { type: "null" }] }, aiScheduleStyle: { anyOf: [{ type: "string", enum: ["daily-dayparts", "weekly-broadcast", "endless-rotation"] }, { type: "null" }] } } },
      SelectChannelContentRequest: { type: "object", required: ["sourceIds", "instructions"], additionalProperties: false, properties: { sourceIds: { type: "array", minItems: 1, maxItems: 20, items: { type: "string", minLength: 1 } }, instructions: { type: "string", minLength: 1, maxLength: 4000 }, targetCount: { type: "integer", minimum: 1, maximum: 200 }, aiProvider: { anyOf: [{ type: "string", enum: ["anthropic", "openai"] }, { type: "null" }] } } },
      CreateChannelFromBriefRequest: { type: "object", required: ["name", "templateId", "brief", "sourceIds"], additionalProperties: false, properties: { name: { type: "string", minLength: 1, maxLength: 160 }, templateId: { type: "string", minLength: 1 }, brief: { type: "string", minLength: 1, maxLength: 4000 }, sourceIds: { type: "array", minItems: 1, maxItems: 20, items: { type: "string", minLength: 1 } }, scheduleStyle: { anyOf: [{ type: "string", enum: ["daily-dayparts", "weekly-broadcast", "endless-rotation"] }, { type: "null" }] }, aiProvider: { anyOf: [{ type: "string", enum: ["anthropic", "openai"] }, { type: "null" }] } } },
