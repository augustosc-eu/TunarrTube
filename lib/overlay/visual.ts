...
function positionStyle(element: VisualElement): string {
  return [
    "position:absolute",
    `left:${element.x}px`,
    `top:${element.y}px`,
...
function renderElement(element: VisualElement): string {
  if (element.kind === "image") {
    const style = [positionStyle(element), `opacity:${element.opacity ?? 1}`, "object-fit:contain"].join("; ");
    return `  <img src="${escapeAttribute(element.src ?? "")}" style="${style}" />`;
  }
  return `  <div style="${elementStyle(element)}">{{${element.bindingKey}}}</div>`;
}
...
// goes read-only whenever a layout is present; see components/template-editor.tsx). Image elements
// have no binding key/value -- their pixels are baked into the generated HTML directly -- so they're
// skipped here.
export function layoutToBindings(layout: VisualLayout): BindingField[] {
  const seen = new Set<string>();
  const bindings: BindingField[] = [];
  for (const element of layout.elements) {
    if (element.kind === "image" || !element.bindingKey || seen.has(element.bindingKey)) continue;
