...
11. **TunarrTube only *analyzes* public HTTPS YouTube URLs** on the hosts allow-listed in `lib/youtube/url.ts` — it can't browse a signed-in account's library or accept a private/unlisted URL it wasn't given. Cookie-based `yt-dlp` auth (`AppSettings.ytdlpCookiesPath`/`TUNARRTUBE_YTDLP_COOKIES`, wired via `cookiesArgs()` in `lib/youtube/ytdlp.ts`) is supported, but stays file-path-only and opt-in: the app never accepts, stores, or displays cookie/account credentials directly, only a path to a file the operator manages and mounts in themselves. Follow that shape for any future auth-adjacent feature — don't add a flow where credentials pass through the app's own request/response bodies or land in the DB — and discuss the security implications before broadening it further.
