...
### Cookie-based YouTube auth is opt-in, file-path-based, and never touches the DB or logs
**Explicit** (constraint) **/ Evidenced** (mechanism). `AGENTS.md`: cookie-based `yt-dlp` auth must "stay file-path-only and opt-in" and never let "credentials pass through the app's own request/response bodies or land in the DB." The MVP shipped with none; `AppSettings.ytdlpCookiesPath` (nullable, defaulting to unset) adds it as opt-in: an operator points it at an absolute path to a Netscape-format `cookies.txt` file they generate and mount into the container themselves (e.g. a `compose.yaml` bind mount) — TunarrTube never accepts, stores, or displays cookie *contents*, only the path, which `lib/youtube/ytdlp.ts:cookiesArgs` passes through as `--cookies <path>` to every `yt-dlp` invocation and `lib/downloads/service.ts:downloadMp4`. `lib/settings/service.ts:validateCookiesPath` only checks the path is absolute and currently readable when set via Settings; it isn't re-validated on every `getSettings()` call the way `mediaBaseDirectory` is, so a file that later goes missing (an unmounted volume, etc.) surfaces as an ordinary `yt-dlp` failure rather than blocking app startup. `--cookies <path>` is redacted by the pre-existing `sanitizeLogValue`/`safeCommand` regexes (see the sanitization decision above), which already targeted `--cookies[-from-browser]` before any caller passed that flag — evidence this was anticipated, not accidental.

### "Sign in to confirm your age/you're not a bot" is a distinct, actionable failure from "video is gone" -- but not (yet) a distinct retry policy
**Evidenced.** `lib/youtube/ytdlp.ts:isSignInRequiredError` matches yt-dlp's `Sign in to confirm...` messages separately from `isUnavailableVideoError`'s `private|unavailable|deleted|removed`. Both cause `downloadVideo`/`cacheVideo`/`enrichVideo` to record `Video.availability = "unavailable"` with an explanation (`unavailabilityReason`, pointing sign-in-required failures at the Settings cookies field rather than yt-dlp's own `--cookies-from-browser` hint, not applicable in a headless container) and cause `syncSource` to stop queuing fresh download attempts for that video on future syncs. Unlike a more elaborate job-level "fail immediately, don't retry" mechanism, a `download`/`cache` job hitting either signal still goes through the ordinary per-job retry-with-backoff in `jobs/runner.ts` before landing `"failed"` — a smaller, self-contained change than adding a new job-outcome class, left as a natural follow-up if `jobs/runner.ts` grows one.
