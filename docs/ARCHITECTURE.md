...
| `GET/POST /api/templates`, `GET/PATCH /api/templates/[id]` | `ensureBuiltInTemplates` + inline Prisma | `lib/overlay/service.ts` / `lib/db/client.ts` |
...
- **`overlay/service.ts`** — `resolveBindingValues` (the single source of truth for a render's live text: an explicit `MediaItem.customFieldsJson` entry, then a built-in column via `BUILT_IN_FIELD_ALIASES`, then the binding's own `sampleValue` only for keys with no alias), `renderOverlayLayers` (Puppeteer screenshot per timed layer, written atomically under `storage/render-cache/`), `ensureBuiltInTemplates` (self-healing seed of the two built-in templates, same convention as `getSettings()`).
- **`overlay/visual.ts`** / **`overlay/visual-types.ts`** — the drag-and-drop template builder's data model (`VisualLayout`/`VisualElement`); `renderLayoutToHtml`/`layoutToBindings` regenerate `htmlTemplate`/`bindingsJson` wholesale from a layout on every visual edit.
