...
- `lib/jobs/runner.ts` — `ytarrWorker` (the in-flight worker promise), `ytarrRecovered`, `ytarrRecovery`, `ytarrWakeTimer`.
...
- **`metadata/service.ts`** — `enrichVideo`: background job that re-fetches full `yt-dlp` metadata for a video and marks it `metadataStatus: "complete"`/`"failed"`, or `availability: "unavailable"` if the error matches `isUnavailableVideoError` (private/unavailable/deleted/removed) or `isSignInRequiredError` (age-restricted/bot-checked, `lib/youtube/ytdlp.ts`).
...
- **`youtube/ytdlp.ts`** — all `yt-dlp` invocations: `analyzePlaylist`, `analyzeChannelFeed`/`analyzeSource` (merges videos/shorts/live when `feedType: "all"`), `fetchVideoMetadata`, `resolveStreamUrl` (takes the effective `VideoQuality`, validates the resolved URL is actually a `youtube.com`/`googlevideo.com` HTTPS host before returning it). `cookiesArgs()` appends `--cookies <path>` to every one of these (and to `downloadMp4` in `lib/downloads/service.ts`) when `AppSettings.ytdlpCookiesPath` is set — see the cookie-auth decision in `docs/DECISIONS.md`. `isSignInRequiredError` classifies yt-dlp's age-restriction/bot-check "Sign in to confirm..." messages distinctly from `isUnavailableVideoError`'s "gone for good" signal; `unavailabilityReason` builds the `Video.availabilityReason` shown in the UI for either case, pointing sign-in-required failures at the Settings cookies field rather than yt-dlp's own `--cookies-from-browser` hint (not applicable in a headless container).
...
| `TUNARRTUBE_YTDLP_COOKIES` | `lib/settings/service.ts` (first-ever creation only; not validated at boot — see the cookie-auth decision in `docs/DECISIONS.md`) | none (unauthenticated `yt-dlp`) |
...
- **Log/error sanitization** (`lib/logging/service.ts:sanitizeLogValue`). Applied to every `writeLog` call and to `runProcess`'s failure messages (`lib/system/process.ts`); redacts signed `googlevideo`/`youtube` URLs and `--cookies[-from-browser] <value>` flags so short-lived authenticated stream URLs and credentials never land in the persisted `LogEntry` table or bubble into an `AppError` message. `--cookies <path>` (see `AppSettings.ytdlpCookiesPath`/`cookiesArgs`, `lib/youtube/ytdlp.ts`) is covered by the same regex.
...
`stopJob` persists cancellation before signalling the worker, including when a running row has no live controller. The worker checks persisted status before dispatch and only writes completion or failure for a running job, so late results cannot overwrite cancellation. `recoverJobs` uses a shared promise and requeues interrupted jobs only below their retry limit; exhausted jobs fail with a recovery log entry for manual retry. Cancelled jobs are left alone. Publishing threads cancellation through `materializeForTunarr` into cache downloads and Tunarr requests; scan polling checks already-aborted signals and removes listeners after each delay.
