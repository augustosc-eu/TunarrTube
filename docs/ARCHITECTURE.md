...
app/                     Next.js App Router
  ...
  openapi.json/route.ts    OpenAPI 3.1 discovery document
...
lib/                     Domain services — the actual application logic
  ...
  channels/, overlay/, renders/, ffmpeg/, sidecar/, metadata-lookup/,
  ingest/, media-items/          Channel-generator domain (see below)
...
| `/channels` | `app/channels/page.tsx` | List curated overlay channels |
| `/channels/new` | `app/channels/new/page.tsx` | Create a channel (name + overlay template) |
| `/channels/[id]` | `app/channels/[id]/page.tsx` | Channel detail: add media, render, publish |
| `/channels/[id]/items/[itemId]` | `app/channels/[id]/items/[itemId]/page.tsx` | Per-item metadata editor + overlay preview |
| `/templates` | `app/templates/page.tsx` | List overlay templates (seeds the built-ins on first load) |
| `/templates/[id]` | `app/templates/[id]/page.tsx` | Visual/code overlay template editor |
...
| `GET/POST /api/logs` | inline Prisma query (`?category=`, capped at 500) / `purgeLogs` | `lib/db/client.ts` / `lib/logging/service.ts` (`{ action: "purge" \| "clear" }` — "purge" deletes entries older than `AppSettings.logRetentionDays`, "clear" deletes all) |
| `GET /api/health` | inline: `SELECT 1` + `schedulerStatus()` | `lib/jobs/scheduler.ts` — used by the Docker `HEALTHCHECK` |
| `GET/POST /api/channels`, `GET/PATCH/DELETE /api/channels/[id]` | `listChannels`/`createChannel`/`getChannel`/`updateChannel`/`deleteChannel` | `lib/channels/service.ts` |
| `POST/DELETE /api/channels/[id]/items` | `attachExistingVideo`/`addYoutubeUrlToChannel`/`enqueueChannelJob("ingest_local_scan", ...)` | `lib/channels/service.ts` |
| `POST /api/channels/[id]/reorder` | `reorderChannelItems` | `lib/channels/service.ts` |
| `POST /api/channels/[id]/render` | `enqueueChannelJob("render", ...)` per pending item | `lib/channels/service.ts` |
| `GET/POST/DELETE /api/channels/[id]/publish` | `channelTunarrLinkStatus`/`enqueueChannelJob("channel_publish", ...)`/`unlinkChannelFromTunarr` | `lib/tunarr/channel-service.ts` |
| `GET/POST /api/templates`, `GET/PATCH/DELETE /api/templates/[id]` | `ensureBuiltInTemplates` + inline Prisma / `deleteTemplate` | `lib/overlay/service.ts` / `lib/db/client.ts` |
| `GET/PATCH /api/media-items/[id]` | `getMediaItem`/`updateMediaItemMetadata` | `lib/media-items/service.ts` |
| `POST /api/media-items/[id]/render` | `enqueueChannelJob("render", ...)` | `lib/channels/service.ts` |
| `GET /api/media-items/[id]/metadata-search`, `POST .../metadata-apply` | `searchMetadata`/`applyMetadata` | `lib/metadata-lookup/service.ts` |

`GET /openapi.json` serves the code-owned OpenAPI 3.1 contract from `lib/openapi/document.ts`. It covers every route and method, including binary playback and thumbnail responses, along with request constraints, shared response envelopes, and the unauthenticated trusted-network boundary. `tests/openapi.test.ts` prevents a route method from being added without an entry in the contract and enforces unique `operationId` values for generated clients.
...
- `lib/jobs/scheduler.ts` — `ytarrSchedulerTimer`, `ytarrCacheTimer`, `ytarrLogPurgeTimer`, `ytarrSchedulerStartedAt`, `ytarrSchedulerRunning`.
...
- **`logging/service.ts`** — `writeLog` (persists a `LogEntry`), `sanitizeLogValue` (redacts signed googlevideo/youtube URLs and `--cookies[-from-browser]` flags — used both here and in `system/process.ts`), `purgeLogs(retentionDays, clear?)` (deletes `LogEntry` rows older than `retentionDays`, or every row when `clear` is set; takes the retention window as a parameter rather than importing `settings/service.ts` directly, avoiding a circular import since that module already imports `writeLog` from here). Run automatically — hourly from `jobs/scheduler.ts` using `AppSettings.logRetentionDays` — and on demand via `POST /api/logs`.
...
- **`channels/service.ts`** — CRUD for `Channel`, `reorderChannelItems`, `addMediaItemToChannel`/`removeChannelItem`, `attachExistingVideo` (wraps an already-downloaded `SourceVideo` as a `MediaItem`), `addYoutubeUrlToChannel` (see below), `enqueueChannelJob` (a `Channel`/`MediaItem`-scoped sibling to `sources/service.ts:enqueueUniqueJob`, kept separate rather than widening that function's signature).
- **`channels/materialize.ts`** — `materializeRenderForChannel`: hardlinks (falling back to copy) a completed `RenderedAsset` into the channel's `storageDirectory` as `<mediaItemId>.mp4`, plus a Kodi `<musicvideo>` NFO and poster via `lib/sidecar/nfo.ts`.
- **`overlay/service.ts`** — `resolveBindingValues` (the single source of truth for a render's live text: an explicit `MediaItem.customFieldsJson` entry, then a built-in column via `BUILT_IN_FIELD_ALIASES`, then the binding's own `sampleValue` only for keys with no alias), `renderOverlayLayers` (Puppeteer screenshot per timed layer, written atomically under `storage/render-cache/`), `ensureBuiltInTemplates` (self-healing seed of the two built-in templates, same convention as `getSettings()`), `deleteTemplate` (refuses a built-in template or one still assigned to a `Channel`).
- **`overlay/visual.ts`** / **`overlay/visual-types.ts`** — the drag-and-drop template builder's data model (`VisualLayout`/`VisualElement`, `kind: "text" | "image"` — absent `kind` means "text", so layouts saved before image elements shipped still parse); `renderLayoutToHtml`/`layoutToBindings` regenerate `htmlTemplate`/`bindingsJson` wholesale from a layout on every visual edit. An image element's `src` is a `data:` URI inlined straight into `htmlTemplate` — no separate asset storage/serving — so it renders as one static frame (an animated GIF freezes on whichever frame Puppeteer paints); it carries no binding and is excluded from `layoutToBindings`.
- **`overlay/puppeteer.ts`** — a lazy-launched, `globalThis`-cached Puppeteer `Browser` singleton (same survives-dev-reload rationale as `lib/db/client.ts`), `renderHtmlToPng` (forces a transparent page background so the screenshot composites via FFmpeg's overlay filter).
- **`renders/service.ts`** — `renderMediaItem`: resolves the source file (a local path, or a `SourceVideo` that must have `downloadStatus: "complete"`), dedups against an existing `complete` `RenderedAsset` for the same `(mediaItem, template)` pair, then calls `lib/ffmpeg/compose.ts:renderVideoWithOverlay`.
- **`ffmpeg/compose.ts`** — `buildOverlayFilterGraph`/`renderVideoWithOverlay`: one `-loop 1` PNG input per timed layer sharing the source video's global clock, `scale`+`fade` per layer chained through sequential `overlay` filter stages, atomic temp-then-`rename` output.
- **`ffmpeg/probe.ts`** + **`ffmpeg/ffprobe-binary.ts`** — `ffprobeMediaInfo` (width/height/duration/codecs) with its own standalone `ffprobe` binary discovery, deliberately not folded into `lib/system/binaries.ts`'s `BinaryName` union.
- **`sidecar/nfo.ts`** — `buildMusicVideoNfo`/`writeMusicVideoNfo` (Kodi `<musicvideo>` XML; `<artist>` always defaults to "Unknown Artist" — a live-verified Tunarr `music_videos` scanner quirk where a missing `<artist>` silently drops the whole file), `writePosterArtwork`, `grabFallbackPoster` (mid-clip FFmpeg frame grab when no metadata-lookup artwork exists).
- **`metadata-lookup/service.ts`** — `searchMetadata` (MusicBrainz + iTunes in parallel via `Promise.allSettled`, merged and sorted by relevance score), `applyMetadata`. `metadata-lookup/musicbrainz.ts` throttles to ~1 req/s and requires `AppSettings.musicbrainzContactEmail` in its `User-Agent`; `metadata-lookup/itunes.ts` searches `entity=musicVideo` since MusicBrainz's data is audio-centric.
- **`ingest/local-scan.ts`** — `scanLocalFolder`: reads a folder for video files, best-effort `ffprobe`s each new one, creates `MediaItem{originType:"local"}` rows, reattaches already-tracked files by exact `originLocalPath` match.
- **`media-items/service.ts`** — `getMediaItem`, `updateMediaItemMetadata` (manual edits clear lookup provenance), `applyMetadataCandidate`.
- **`tunarr/channel-service.ts`** — `publishChannelToTunarr`, `channelTunarrLinkStatus`, `unlinkChannelFromTunarr`: the same "register local media source → scan → match by filename → order lineup → create/update channel → replace programming" flow as `tunarr/service.ts:publishSourceToTunarr`, kept in its own file with its own small copies of the matching/ordering helpers rather than exporting (and coupling to) that file's private ones. Registers the local media source with Tunarr's `music_videos` scanner (`TunarrApiClient.createMusicVideoLocalMediaSource`, a sibling to `createLocalMediaSource` added for this — Sources still use `other_videos`).
...
- **`AppSettings`** — singleton, `id` pinned to `1`: `mediaBaseDirectory`, `tunarrUrl`, `cacheMaxMegabytes` (default 20480), `cacheMaxAgeDays` (default 30), `logRetentionDays` (default 30 — how long `LogEntry` rows are kept before `jobs/scheduler.ts`'s hourly `purgeLogs` call deletes them), `defaultVideoQuality` (default `"best"`), `jobsPaused` (default `false` — the queue-wide pause toggle, checked by `jobs/runner.ts`'s `work()`/`kickWorker()`).
...
- **`LogEntry`** — sanitized operational log, `level`/`category`/`message`/`details`, optional `sourceId`/`videoId`/`channelId`/`mediaItemId` (all `onDelete: SetNull`).
- **`Channel`** — a curated, overlay-rendered lineup (distinct from a `Source`'s own 1:1 Tunarr channel). `templateId` (FK to `OverlayTemplate`), `storageDirectory` (under `<mediaBaseDirectory>/_channels/<slug>`, namespaced so it can never collide with a `Source.directoryName`), `programmingOrder`, `intakeSourceId` (its companion collection `Source`'s id, set lazily — see "Channels: reusing the Source/download pipeline" below), and its own `tunarr*` linkage fields mirroring `Source`'s.
- **`MediaItem`** — a clip curated onto one or more `Channel`s via `ChannelItem`. `originType` is `"sourceVideo"` (a thin, unique pointer at a `SourceVideo` this app already downloaded — no YouTube fields duplicated here) or `"local"` (`originLocalPath`, added by folder scan). Carries music-video-shaped metadata (`artist`/`album`/`year`/`genre`/`releaseDate`/`customFieldsJson`) and `metadataStatus` (`pending`/`manual`/`matched`/`failed`).
- **`ChannelItem`** — thin `(channelId, mediaItemId, position)` join, unique on the pair.
- **`OverlayTemplate`** — an HTML/CSS design with `{{binding}}` placeholders (`htmlTemplate`, `bindingsJson`, `layersJson`), optionally also carrying `visualLayoutJson` (present only while authored/editable via the drag-and-drop builder; `null` means hand-written or detached by a direct HTML edit).
- **`RenderedAsset`** — one burned-in render of a `(mediaItemId, templateId)` pair (unique constraint), shared/hardlinked across every channel using that same clip+template.

Eight committed migrations trace the schema's evolution (`prisma/migrations/`), most recently the channel-generator domain (`Channel`/`MediaItem`/`ChannelItem`/`OverlayTemplate`/`RenderedAsset` plus additive `Job`/`LogEntry`/`AppSettings` columns) on top of the initial schema, Tunarr integration, Phase 2 playback/cache support, video availability reasons, per-source video quality, jobs-paused, and log-retention migrations.
...
- **`next.config.ts`**: `output: "standalone"` (self-contained server output for Docker); `outputFileTracingExcludes` keeps `storage/`, the SQLite files, and the webpack build cache (`.next/cache/`) out of the traced output — it must *not* exclude `.next/` more broadly, since that also matches `.next/server/chunks/*.js`, which `webpack-runtime.js` `require()`s at runtime and the standalone build then fails to copy, producing `Cannot find module './chunks/<id>.js'` on a clean build (confirmed by reproducing it locally: a prior version of this pattern, `./.next/**/*`, silently dropped 22 of 27 chunk files from `.next/standalone/.next/server/chunks/`); `serverExternalPackages: ["@prisma/client"]`; `images.remotePatterns` allow-lists the YouTube/Google thumbnail hosts; global response headers provide browser hardening.
- **`Dockerfile`**: three-stage build (`deps` → `builder` → `runner`). The dependency stage installs OpenSSL and copies `prisma/schema.prisma` before `npm ci` because Prisma Client generation runs in `postinstall`; it also sets `PUPPETEER_SKIP_DOWNLOAD=true` so that `npm ci` doesn't fetch Puppeteer's own bundled Chromium (which would otherwise fail the build outright if either its `chrome` or `chrome-headless-shell` download failed — see `node_modules/puppeteer/lib/esm/puppeteer/node/install.js`) or leave an unused browser behind in a stage the runner never copies from. The runner stage installs `ffmpeg`, `yt-dlp` (via `apt`/`pip3 --break-system-packages`), and Debian's own `chromium` package (apt resolves its ~20 runtime shared-library dependencies automatically) directly into the image, sets `YTARR_PUPPETEER_EXECUTABLE_PATH=/usr/bin/chromium` so `lib/overlay/puppeteer.ts` uses it, creates a non-root `tunarrtube` user/group (uid/gid `1001`), copies the Next.js standalone output plus `prisma/` and `node_modules`, exposes port `3000`, defines a `HEALTHCHECK` against `/api/health`, and its `CMD` runs `prisma migrate deploy` then `node server.js`.
...
- **Scheduler** (`lib/jobs/scheduler.ts`). A `setInterval(60_000)` loop that enqueues due `sync` jobs, plus an hourly `enforceCachePolicy()`, an hourly `purgeLogs()`, and a startup `reconcileCacheFiles()`. Guarded by a `globalThis` flag so concurrent invocations (e.g. across a dev-mode HMR reload) don't run twice.
...
- **Channels: reusing the Source/download pipeline instead of a second YouTube downloader** (`lib/channels/service.ts:addYoutubeUrlToChannel`/`ensureChannelIntakeSource`). Every `Channel` lazily gets its own companion `Source` (`sourceType: "collection"`, `playbackMode: "download"`, `Channel.intakeSourceId`) the first time a YouTube URL is pasted onto it — a real, ordinary `Source`, visible and manageable in the existing Sources UI like any other, not a hidden implementation detail. Pasting a URL calls the existing, unmodified `addVideosToCollection` (`lib/sources/service.ts`) against that intake source, which runs the video through the normal metadata+download job pipeline; once its `SourceVideo.downloadStatus` reaches `"complete"`, the channel-side code wraps it as a `MediaItem{originType:"sourceVideo", sourceVideoId}`. Picking an already-downloaded video from *any* existing `Source` (`attachExistingVideo`) works the same way, minus the download. This means `lib/sources/service.ts`, `lib/downloads/service.ts`, and `lib/tunarr/service.ts` needed zero code changes to support Channels — the channel-generator domain's job types (`render`, `ingest_local_scan`, `channel_publish`) are additive `if` branches appended to `lib/jobs/runner.ts:handleJob()`, which automatically fall into `claimJob()`'s existing "everything but download/retag" priority tier without any tuning.
