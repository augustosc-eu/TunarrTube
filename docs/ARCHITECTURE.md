...
1. **`yt-dlp`** — invoked exclusively via `lib/system/process.ts:runProcess`, wrapped by `lib/youtube/ytdlp.ts`. Used for: playlist/channel analysis (`--dump-single-json --flat-playlist`), single-video metadata refresh, permanent video download (`-f bv*[vcodec^=avc]+ba[acodec^=mp4a]/b[ext=mp4]/best --merge-output-format mp4 --recode-video mp4 --embed-metadata`, via `--ffmpeg-location`), and resolving a direct stream URL for `stream`-mode playback (`--get-url`). `--embed-metadata` embeds title/description as container tags (harmless for other players, but **not** what makes Tunarr show a real title — see below). Binary location is resolved by `lib/system/binaries.ts:discoverBinary` (env override → `which` → fallback paths).
