...
- **`app/layout.tsx`** — root layout; renders `Sidebar` plus `{children}` inside a `.shell` div. Sets the `<title>` template (`"%s · YTarr"`), applies the `next/font/google` Inter variable to `<html>`, and renders a `beforeInteractive` inline `Script` (see "Theming" below) that sets `<html data-theme>` before hydration.
...
- **`downloads/service.ts`** — `downloadVideo` (permanent download via `yt-dlp`+FFmpeg into the source's directory, with hardlink/copy reuse of an identical file already downloaded for another source via `reuseExistingAsset`), `cacheVideo` (same download machinery, targeting the shared cache directory), `materializeForTunarr` (promotes a cache/stream video to a real file in the source directory so Tunarr can scan it), `retagVideo` (re-embeds the current `Video.title`/`description`/`uploader` into an already-downloaded MP4 in place via an `ffmpeg -c copy` remux, for videos downloaded before metadata embedding existed), `touchCacheAsset`.
- **`jobs/runner.ts`** — the single background worker: `recoverJobs()` requeues anything left `running`/`downloading` from a previous crash; `claimJob()` atomically claims the oldest due `queued` job (`updateMany` with a `status: "queued"` guard, `attempts: increment`); `handleJob()` dispatches by `job.type` to the relevant service function (`metadata`, `thumbnail`, `sync`, `download`, `cache`, `retag`, `tunarr_publish`, `tunarr_refresh`); `work()` loops until no jobs remain, retries failures with capped exponential backoff (`min(60, 2^attempts * 2)` seconds) up to `job.maxAttempts` (3, except `tunarr_refresh` = 100), and re-enqueues a `tunarr_refresh` job after any `download`/`cache`/`retag` job completes for a source with a linked Tunarr channel. `kickWorker()` starts/wakes the loop and self-schedules a wake timer for the next due job.
...
1. **`yt-dlp`** — invoked exclusively via `lib/system/process.ts:runProcess`, wrapped by `lib/youtube/ytdlp.ts`. Used for: playlist/channel analysis (`--dump-single-json --flat-playlist`), single-video metadata refresh, permanent video download (`-f bv*[vcodec^=avc]+ba[acodec^=mp4a]/b[ext=mp4]/best --merge-output-format mp4 --recode-video mp4 --embed-metadata`, via `--ffmpeg-location`), and resolving a direct stream URL for `stream`-mode playback (`--get-url`). `--embed-metadata` embeds title/description/uploader/upload-date as container tags so Tunarr's own library scan (which reads files, not YTarr's API — see below) shows a real title instead of the bare YouTube ID; `lib/downloads/service.ts:retagVideo` re-applies the same tags to a file already on disk via a cheap `ffmpeg -c copy` remux, for a Settings-triggered backfill over videos downloaded before this existed. Binary location is resolved by `lib/system/binaries.ts:discoverBinary` (env override → `which` → fallback paths).
