...
function Section({ title, jobs, now }: { title: string; jobs: Job[]; now: number }) {
  if (!jobs.length) return null;
  return <><h2>{title} <span className="muted">({jobs.length})</span></h2><div className="table-wrap" style={{ marginBottom: 24 }}><table><thead><tr><th>Job</th><th>Target</th><th>Status</th><th>Detail</th></tr></thead><tbody><JobRows jobs={jobs} now={now} /></tbody></table></div></>;
}
...
export function JobQueue({ initial }: { initial: QueueData }) {
  ...
  return <>
    ...
    {isEmpty
      ...
      : <>
        <Section title="Running" jobs={data.running} now={now} />
        <Section title="Queued" jobs={data.queued} now={now} />
        <Section title="Recent" jobs={data.recent} now={now} />
      </>}
  </>;
}
