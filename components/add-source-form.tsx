...
type Draft = { id: string; name: string; uploaderName: string | null; thumbnailUrl: string | null; videoCount: number; sourceType: "playlist" | "channel"; feedType: string; historyLimit: number | null };
...
export function AddSourceForm() {
  ...
  async function create() {
    ...
    try {
      const source = await data(await fetch("/api/sources", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ draftId: draft.id, name, playbackMode: mode, syncEnabled, syncIntervalMinutes: Number(interval) }) }));
  ...
  return <div className="card form-card"><form onSubmit={analyze}>
    <div className="field"><label htmlFor="youtube-url">YouTube playlist or channel URL</label><input className="input" id="youtube-url" type="url" required placeholder="https://www.youtube.com/@channel" value={url} onChange={(event) => { setUrl(event.target.value); setDraft(null); }} /></div>
  ...
  </form>{error ? <div className="error" role="alert">{error}</div> : null}{draft ? <section className="preview">{draft.thumbnailUrl ? <Image className="source-thumb" src={draft.thumbnailUrl} width={240} height={180} alt="" /> : <div className="source-thumb placeholder">YT</div>}<div>
