...
type Row = { membershipId: string; videoId: string; youtubeId: string; title: string; uploader: string | null; durationSeconds: number | null; playlistIndex: number | null; metadataStatus: string; availability: string; membershipStatus: string; downloadStatus: string };
...
export function VideoSelectionTable({ sourceId, rows }: { sourceId: string; rows: Row[] }) {
  ...
  return <>{playing ? <div className="card player"><div className="toolbar"><strong>{playing.title}</strong><span className="spacer"/><button className="button secondary" onClick={() => setPlaying(null)} aria-label="Close player"><X size={15}/></button></div><video controls autoPlay src={`/api/playback/${sourceId}/${playing.videoId}`} /></div> : null}<div className="toolbar"><button className="button" disabled={busy || selected.size === 0} onClick={download}><Download size={15} /> Download selected ({selected.size})</button><span className="muted">{downloadable.length} available to download</span></div>{error ? <div className="error">{error}</div> : null}<div className="table-wrap"><table><thead><tr><th><input type="checkbox" aria-label="Select all downloadable videos" checked={downloadable.length > 0 && downloadable.every((row) => selected.has(row.videoId))} onChange={(event) => setSelected(event.target.checked ? new Set(downloadable.map((row) => row.videoId)) : new Set())} /></th><th>#</th><th>Video</th><th>Duration</th><th>Metadata</th><th>Download</th><th>Play</th></tr></thead><tbody>{rows.map((row) => <tr key={row.membershipId}><td><input type="checkbox" disabled={row.downloadStatus === "complete" || row.membershipStatus !== "present"} checked={selected.has(row.videoId)} onChange={() => toggle(row.videoId)} aria-label={`Select ${row.title}`} /></td><td>{row.playlistIndex ?? "—"}</td><td className="title-cell"><strong>{row.title}</strong><span className="meta">{row.uploader ?? row.youtubeId}{row.membershipStatus === "missing" ? " · Missing from source" : ""}</span></td><td>{duration(row.durationSeconds)}</td><td><span className={`badge ${row.availability === "unavailable" ? "unavailable" : row.metadataStatus}`}>{row.availability === "unavailable" ? "unavailable" : row.metadataStatus}</span></td><td><span className={`badge ${row.downloadStatus}`}>{row.downloadStatus.replaceAll("_", " ")}</span></td><td><button className="button secondary" disabled={preparing === row.videoId || row.membershipStatus !== "present"} onClick={() => play(row)}>{preparing === row.videoId ? <LoaderCircle size={14} className="animate-spin"/> : <Play size={14}/>}</button></td></tr>)}</tbody></table></div></>;
}
